...
- `app/page.tsx` — main dashboard (summary, filters, paged list, add form, receipt upload, chat widget).
...
- `lib/` — `auth.ts` (scrypt + session), `workspace.ts` (`getWorkspaceId`), `apiFetch.ts` (workspace header), `openai.ts`, `importCsv.ts` (CSV row validation), `format.ts` (`fmtCurrency`: `id-ID`/`IDR`), `types.ts`, `ui.ts` (`ICON_BTN` 44px tap target).
