...
- `GET /api/insights` returns `{ insights: [{ kind, title, detail }], periodLabel, source }` over the trailing 3 calendar months: one cached AI insight per workspace per day (`daily_insights` table) when an `OPENAI_API_KEY` is set, else the rules engine in `lib/insights.ts` (pure `buildInsights`, prompt via `summarizeForAI`/`buildInsightPrompt`, strict `parseAiInsights` — null means fall back, silently, to rules). `generateAiInsights` (`app/api/insights/route.ts`) calls the model with `max_tokens: 6000` — reasoning models (e.g. `muse-spark-1.3-contributor`) spend thousands of hidden `reasoning_tokens` before any visible `content`; too low a budget returns `content: null`/`finish_reason: "length"` and silently falls back to rules. `buildInsightPrompt` is written in Bahasa Indonesia and instructs the model to reply in Bahasa Indonesia — the rules-engine fallback text in `buildInsights` stays English. `GET /api/insights?force=1` skips the daily-cache read (but still overwrites the cache with the fresh result) — `InsightsPanel`'s manual refresh button uses it; the plain `tick`/workspace-switch autoload does not, so reloads keep serving today's cached insight until the button is used again. `InsightsPanel` refetches on workspace switch and on every data change via the `tick` prop from `app/page.tsx`, renders nothing on fetch failure, and has a manual refresh button (loading state, plus a "Get insight" CTA in the empty state).
